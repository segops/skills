...
If not, run `segops-integrate-node`, `-react`, or `-nextjs` first.
...
- **Server:** a shared `segops` instance (see `segops-integrate-node`).
...
The `segops-admin-script` skill covers more Admin SDK tasks if you want to script
