...
> For **Next.js**, use `integrate-react`'s sibling skill
> `integrate-nextjs` instead — it handles the App/Pages Router split and
...
the `integrate-node` skill or `@segops/sdk/server`'s `signUserId`):
